...
class Blockchain extends React.Component {
  createBlocks(item) {
    return (
      <div className="block" >
        <Card title={(item.key === '0') ? 'GENESIS BLOCK':('BLOCK #' + item.key)} hoverable = "true" style={{borderRadius: '25px'}} >
          <Input
            addonBefore={'Data'}
            ...
            <div style={{padding: '10px', flexWrap: 'nowrap', overflow: 'auto', justifyContent: 'flex-start'}}>
              ...
              <span>TIME PUBLISHED: <Tag color='magenta'> {item.time} </Tag> </span> <br/>
            </div>
        </Card>
        <Icon type="down-circle" theme="twoTone" style={{ fontSize: '36px', width: '100%', margin: '20px auto' }} />
      </div>
      );
